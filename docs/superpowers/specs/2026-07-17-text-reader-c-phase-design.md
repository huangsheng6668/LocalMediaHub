...
B 阶段已经实现 txt/epub 阅读核心：章节切分、翻页、章节级进度持久化、TOC 抽屉、HomeScreen 书架、Web 阅读器。但排版/主题/交互全是硬编码——`MaterialTheme.typography.bodyLarge`、固定 1.8 行距、跟随系统暗黑、段落无可交互入口。
...
> 仅用 `material-icons-core` 已有的图标，不触发 `material-icons-extended` 依赖（B 阶段 T12 已踩坑）。
...
### ReaderTheme（新 `ui/component/reader/ReaderTheme.kt`）

```kotlin
@Composable
fun ReaderTheme(theme: ReaderTheme, content: @Composable () -> Unit) {
...
### 自动滚动（`TextReaderViewModel.kt` 扩展）
...
逻辑：
- `toggleAutoScroll()`：
  - off → on：启动协程 `while (isActive) { listState.scrollBy(pixelsPerTick); delay(16) }`，`pixelsPerTick = settings.autoScrollSpeed * 0.5f`（60fps 下相当于 30~300 px/s）
  - on → off：cancel 协程
- 启动时 `Activity.getWindow().addFlags(FLAG_KEEP_SCREEN_ON)`；停止时清除
- 手动翻页（prev/next/loadChapter）→ 自动停止
- 节流写进度：滚动期间每 1 秒最多一次 `saveBookProgress`（debounce 或 timestamp 对比）

`LazyListState` 由 Activity 范围持有（`rememberSaveable` 或 ViewModel 字段），供 VM 调用 `scrollBy`。
...
### TopBar 改造（`textReader.js`）

```
[←返回] [书名]              [Aa] [▶] [☰目录]
```

- **Aa**：打开 `<dialog id="reader-settings-dialog">`（HTML5 `<dialog>` 原生 Modal）
- **▶/⏸**：自动滚动切换（图标互换）
- **☰目录**：保留现有 TOC 抽屉

### 设置面板 DOM（`index.html` + `textReader.js`）
...
`style.css`：
```css
#view-reader .chapter-content {
  background-color: var(--reader-bg, #fff);
  color: var(--reader-fg, #212121);
...
CSS 变量仅作用于 `#view-reader` 容器——App 其他部分不受影响。

### 自动滚动（`textReader.js`）

- ▶ 启动 `requestAnimationFrame` 循环 `el.scrollTop += pixelsPerFrame`，`pixelsPerFrame = speed * 0.5`（与 Android 60fps 等效）
...
- 点击书签 → 关闭抽屉 + `element.scrollIntoView({behavior: 'smooth'})`
...
- 服务端零变更——C 阶段纯客户端
- B 阶段 `book_progress`（chapterIndex + scrollOffsetPx）系统不变——C 阶段书签是平行系统
...
- **LazyListState 上提到 VM 层**：B 阶段 listState 在 Composable 内 `rememberLazyListState()`，C 阶段自动滚动需要 VM 调 `scrollBy`，必须上提。需要验证上提后段落渲染、配置变更（旋屏）行为正常
- **TOC 抽屉 Tab 化重构**：现有抽屉内容较深嵌套，加 TabRow 时要小心 padding/滚动行为
